...
class L1Step(AttackerStep):
    ...
    def step(self, x, g):
        l = len(x.shape) - 1        
        mmax = ch.max(ch.abs(g).view(g.shape[0], -1), dim=1).values / 200
        g_norm = mmax.view(-1, *([1]*l))
        #print(g_norm.shape)
        #g_norm = ch.norm(g.view(g.shape[0], -1), dim=1).view(-1, *([1]*l)) / 6400
        #print(g_norm)
        #print(g)
        #print(g_norm)
...
#        scaled_g = g / (g_norm + 1e-10)
#        print(scaled_g)
#        return x + scaled_g * self.step_size * 3000
...
class L2Step(AttackerStep):
    ...
    def step(self, x, g):
        ...
        g_norm = ch.norm(g.view(g.shape[0], -1), dim=1).view(-1, *([1]*l))
        scaled_g = g / (g_norm + 1e-10)
        return x + scaled_g * self.step_size * 100000
