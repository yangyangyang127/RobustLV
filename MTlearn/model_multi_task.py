...
class HomoMultiTaskModel(object):
    ...
    def __init__(self, num_tasks, network_name, output_num, gpus, file_out, trade_off=1.0, optim_param={"init_lr":0.00003, "gamma":0.3, "power":0.75, "stepsize":3000}):
        ...
        self.networks = [[nn.Linear(self.shared_layers.output_num(), self.output_num).cuda()] for i in range(self.num_tasks)]
        ...
        print(self.networks[0].module)
        print(self.networks[0].module[-1].weight.size())
        weight_size = self.networks[0].module[-1].weight.size()
        all_weights = [self.networks[i].module[-1].weight.view(1, weight_size[0], weight_size[1]) for i in range(self.num_tasks)]
        print(all_weights)
        ...
        print(temp_task_cov_var)
        print(temp_class_cov_var)
        print(temp_feature_cov_var)
...
if __name__ == "__main__":
    ...
    config["model"] = HomoMultiTaskModel(3, "vgg16no_fc", 5, config["gpus"], config["file_out"], trade_off=1, optim_param={"init_lr":0.00003, "gamma":0.3, "power":0.75, "stepsize":1})
