...
class DCAENet(nn.Module):
    def __init__(self):
        ...
        self.bn1 = nn.Sequential(nn.BatchNorm2d(6),
                                nn.LeakyReLU(inplace=True))

        self.convd1 = nn.Sequential(nn.Conv2d(6, 60, kernel_size=7, stride=1, padding=3, dilation=1),
                                    nn.BatchNorm2d(60),
                                   nn.LeakyReLU(inplace=True),
...
def train(crossi, batch_size, epoch, learning_rate, device = torch.device("cuda")):
    ...
    for e in range(0, epoch):
        ...
        if e > 2 and (e % 160 == 0 or e % 270 == 0 or e % 360 == 0 or e % 4350 == 0):
            lr = lr * 0.12
...
if __name__ == "__main__":
    train(0, 50, 310, 0.0005, torch.device("cuda"))
